...
export default function CheckoutPage() {
    ...
    const isGift = searchParams.get('type') === 'gift';
    ...
    const handleSubmit = () => {
        console.log({ planId, isGift, devices, paymentMethod, totalPrice });
        // TODO: передать isGift в запрос на бэкt
    };
 
    // Тексты зависят от типа покупки
    const pageTitle   = isGift ? 'Оформление подарка' : 'Оформление заказа';
    const submitLabel = isGift
        ? 'Подарить'
    ...
    return (
        <div className="checkout">
            <div className="checkout__inner container">
                <h1 className="checkout__title h1">{pageTitle}</h1>
